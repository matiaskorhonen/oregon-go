...
Sensor* Sensor::getRightSensor(char* s) {
  ...
  if (len > 4) {
    ...
    if (strncmp(s, OregonSensorV2::_sensorId,
                strlen(OregonSensorV2::_sensorId)) == 0) {
#ifdef SENSORDEBUG
      printf("Sensor::getRightSensor – create OregonSensorV2\n");
#endif
      return new OregonSensorV2(s);
    } else if (strncmp(s, OregonSensorV3::_sensorId,
                       strlen(OregonSensorV3::_sensorId)) == 0) {
#ifdef SENSORDEBUG
      ...
      std::cout
          << "[Sensor::getRightSensor] right length but unknown signature:" << s
          << std::endl;
    }
#endif
  } else {
    std::cout << "[Sensor::getRightSensor] dont know how to decode: " << s
              << std::endl;
  }
...
bool OregonSensorV2::decode(char* _str) {
  ...
  if (len > 11) {
    ...
    switch (isensorId) {
      ...
      default:
#ifdef SENSORDEBUG
        std::cout << "Unknown sensor id: " << std::hex << isensorId
                  << std::endl;
#endif
...
#ifdef SENSORDEBUG
    printf(
        "OSV2 – decode : id(%s) temp(%s) sign(%c) humid(%s) pressure(%s) "
        "crc(%s)\n",
...
#ifdef SENSORDEBUG
    printf(
        "OSV2 – decode : id(0x%04X) temp(%f) sign(%d) humid(%d) pressure(%d) "
        "cksum(0x%02X) crc(0x%02X)\n",
...
#ifdef SENSORDEBUG
    printf(
        "OSV2 – decode : id(0x%04X) dir(%f) speed(%f) cksum(0x%02X) "
        "crc(0x%02X)\n",
...
#ifdef SENSORDEBUG
    printf(
        "OSV2 – decode : id(%s) ch(%c) bat(%c) temp(%s) sign(%c) humid(%s) "
        "cksum(%s) crc(%s)\n",
...
#ifdef SENSORDEBUG
    printf(
        "OSV2 – decode : id(0x%04X) ch(%d) bat(%d) temp(%f) sign(%d) humid(%f) "
        "cksum(0x%02X) crc(0x%02X)\n",
...
bool OregonSensorV2::decode_THN132N(char* pt) {
  ...
  if (len == 16) {
...
#ifdef SENSORDEBUG
    printf(
        "OSV2 – decode : id(%s) ch(%c) bat(%c) temp(%s) sign(%c) cksum(%s) \n",
    ...
    for (int i = 2; i <= 11; i++)
      _sum += getIntFromChar(pt[i]);
    _sum += getIntFromChar(pt[13]);

#ifdef SENSORDEBUG
    printf(
        "OSV2 - decode : id(0x%04X) ch(%d) bat(%d) temp(%f) sign(%d) "
        "cksum(0x%02X) _chksum(0x%02X)\n",
...
bool OregonSensorV2::validate(char* _str, int _len, int _CRC, int _SUM) {
  ...
  if (SUM == _SUM /* && CRC == _CRC */)
    return true;
  else {
#ifdef SENSORTRACE
    printf(
        "OSV2 – validate err (%s) SUM : 0x%02X(0x%02X) CRC : 0x%02X(0x%02X)\n",
...
#ifdef SENSORDEBUG
    printf(
        "OSV3 – decode : id(%s) ch(%c) bat(%c) temp(%s) sign(%c) humid(%s) "
        "cksum(%s) crc(%s)\n",
...
#ifdef SENSORDEBUG
    printf(
        "OSV3 – decode : id(0x%04X) ch(%d) bat(%d) temp(%f) sign(%d) humid(%f) "
        "cksum(0x%02X) crc(0x%02X)\n",
        0x1D30, ichannel, ibattery, dtemp, itempS, dhumid, ichecksum, icrc);
#endif
...
#ifdef SENSORTRACE
    printf(
        "OSV3 – validate err (%s) SUM : 0x%02X(0x%02X) CRC : 0x%02X(0x%02X)\n",
        _str, SUM, _SUM, CRC, _CRC);
#endif
  }
  return false;
