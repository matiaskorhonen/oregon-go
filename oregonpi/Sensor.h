...
class Sensor {
 protected:
  ...
  virtual bool decode(
      char* _str) = 0;  // decode the string and set the variable

 protected:
  int getIntFromChar(
      char c) const;  // transform a Hex value in char into a number
  int getIntFromString(
      char*) const;  // transform a Hex value in String into a number
  double getDoubleFromString(
      char*) const;  // transform a BCD string into a double
 ...
 public:
  ...
  bool isBatteryLow()
      const;                // return true if valid && haveBattery && flag set.
...
class OregonSensorV2 : public Sensor {
 ...
 private:
  ...
  bool validate(char* _str,
                int _len,
                int _CRC,
                int _SUM);  // Verify CRC & CKSUM
};
