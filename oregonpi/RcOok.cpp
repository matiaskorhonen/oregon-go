...
int RCSwitch_::decode(word duration) {
  ...
  if (changeCount > 6 /* this looks like end */
      && duration > 5000 && duration > this->timings[0] - 200 &&
      duration < this->timings[0] + 200) {
    _changeCount = changeCount - 1;
    changeCount = 0;
    switch (this->nProtocol) {
      case 1:
        if (receiveProtocol1(_changeCount) == true)
          return 1;
        break;
      case 2:
        if (receiveProtocol2(_changeCount) == true)
          return 1;
        break;
    }
    return -1;
...
bool RCSwitch_::receiveProtocol1(unsigned int _changeCount) {
  ...
  if (_changeCount > 6) {  // ignore < 4bit values as there are no devices
